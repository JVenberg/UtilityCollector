...
exports.populateBillReadings = exports.triggerScraperManual = exports.fetchMeterReadings = exports.triggerScraper = exports.sendReminders = exports.sendTestEmail = exports.sendAllInvoices = exports.sendInvoiceEmail = exports.disconnectGmail = exports.gmailOAuthCallback = exports.getGmailAuthUrl = void 0;
...
async function sendEmailViaGmailApi(to, subject, htmlBody, attachments) {
    ...
    if (attachments && attachments.length > 0) {
        ...
        for (const attachment of attachments) {
            emailLines.push(`--${boundary}`, `Content-Type: application/pdf; name="${attachment.filename}"`, `Content-Disposition: attachment; filename="${attachment.filename}"`, `Content-Transfer-Encoding: base64`, ``, attachment.content.toString("base64"));
        }
...
async function getGmailTransporter() {
    ...
    const fakeTransporter = {
        sendMail: async (options) => {
            const messageId = await sendEmailViaGmailApi(options.to, options.subject || "", options.html || options.text || "", options.attachments?.map((a) => ({
...
function generateInvoiceHtml(invoice, billDate, billId, paymentInstructions, hoaName) {
...
async function getPdfAttachment(pdfUrl) {
    try {
...
exports.sendInvoiceEmail = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Generate email HTML
        const html = generateInvoiceHtml(invoice, bill.bill_date, billId, emailSettings.payment_instructions || undefined, emailSettings.hoa_name || undefined);
...
exports.sendTestEmail = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Generate email HTML
        const html = generateInvoiceHtml(sampleInvoice, testBillDate, testBillId, emailSettings.payment_instructions || undefined, emailSettings.hoa_name || undefined);
        // Build subject with optional HOA name
        const subjectPrefix = emailSettings.hoa_name
            ? `[${emailSettings.hoa_name}] `
            : "";
        ...
        throw new functions.https.HttpsError("internal", error instanceof Error ? error.message : "Failed to send test email");
    }
});
                ...
                for (let i = 0; i < reminderDays.length; i++) {
                    ...
                    if (daysSinceSent === reminderDay && remindersSentCount <= i) {
                        ...
                        try {
                            ...
                            remindersSent++;
                            console.log(`Reminder sent to ${invoice.tenant_email} (${daysSinceSent} days), message ID: ${result.messageId}`);
                        }
                        catch (reminderError) {
                            console.error(`Failed to send reminder to ${invoice.tenant_email}:`, reminderError);
                            // Log failed reminder attempt
                            const failedReminderLogEntry = {
                                type: "reminder",
...
exports.fetchMeterReadings = functions
    .runWith({
    ...
    try {
        ...
        if (readingsCount > 0) {
            ...
            for (const [unitNumber, readingData] of Object.entries(readings)) {
                ...
                if (unitInfo) {
                    const gallons = readingData.gallons || 0;
        ...
        throw new functions.https.HttpsError("internal", error instanceof Error ? error.message : "Failed to fetch readings");
    }
});
...
exports.populateBillReadings = functions
    .runWith({
    timeoutSeconds: 120,
    memory: "256MB",
})
